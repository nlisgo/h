...
def group_search(request):
    ...
    if not result.get('q'):
        result['zero_message'] = Markup(
            _('The group &ldquo;{name}&rdquo; have not made any annotations '
              'yet.').format(name=Markup.escape(group.name)))
...
def user_search(request):
    ...
    if not result.get('q'):
        if request.authenticated_user == user:
            result['zero_message'] = _("You haven't made any annotations yet.")

        else:
            result['zero_message'] = _(
                "{name} hasn't made any annotations yet.".format(
